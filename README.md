...
response = client.end_points.get_topics(
    from_date=0,
...
async def main() -> None:
    response = await client.end_points.get_topics(
        from_date=0,
        llm_provider="llmProvider",
        tag_ids=["string"],
        to_date=0,
...
client.with_options(max_retries=5).end_points.get_topics(
    from_date=0,
    llm_provider="llmProvider",
    tag_ids=["string"],
    to_date=0,
...
client.with_options(timeout=5.0).end_points.get_topics(
    from_date=0,
    llm_provider="llmProvider",
    tag_ids=["string"],
    to_date=0,
...
end_point = response.parse()  # get the object that `end_points.get_topics()` would have returned
print(end_point)
```

These methods return an [`APIResponse`](https://github.com/joinanvil/python-sdk/tree/main/src/anvil/_response.py) object.

The async client returns an [`AsyncAPIResponse`](https://github.com/joinanvil/python-sdk/tree/main/src/anvil/_response.py) with the same structure, the only difference being `await`able methods for reading the response content.
...
We are keen for your feedback; please open an [issue](https://www.github.com/joinanvil/python-sdk/issues) with questions, bugs, or suggestions.
